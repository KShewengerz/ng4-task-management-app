...
import { TableName, ProjectField } from "../shared/enums/index";
...
export async function up(knex: Knex) {
  return await knex.schema.createTable(TableName.Project, table => {
    ...
    table.integer(ProjectField.Ordinal).notNullable();
  })
  .then(async () => {
    const projects = [
      {
